...
def add_file_options(parser):
    ...
    file_options.add_argument('-t', '--tmp', help="""Optional argument: where to write 
        temporary files. If not specified, programs-specific tmp will be used.""", default=None)

    file_options.add_argument('--overwrite', help="""Flag: overwrite existing files. 
        Note that certain files may be re-created regardless of the presence or absence 
        of this flag. """, action='store_true')

    file_options.add_argument('-k', '--keep-intermediate-files', help="""Flag: unless this flag is 
        given, all intermediate files (such as discarded reads) will be deleted, unless the
        [--do-not-call] option is also given.""", action='store_true')
...
def add_htseq_options(parser):
    ...
    htseq_options.add_argument('--htseq-options', help="""Optional argument: a space-delimited 
        ...
        If specified, htseq-count options will override default settings.""", nargs='*', type=str)
