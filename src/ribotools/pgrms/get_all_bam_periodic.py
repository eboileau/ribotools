...
# we use rpbp (should be the same as pproc.defaults)
...
def main():
    ...
    parser.add_argument('--do-not-call', help="""If this flag is present, then the program
        will not be executed (but all commands will be printed).""", action='store_true')
    ...
    for sample_name in config['riboseq_samples'].keys():
        ...
        bam_to_filter = filenames.get_riboseq_bam(config['riboseq_data'],
                                                  sample_name,
                                                  is_unique=is_unique,
                                                  note=note)
        ...
        if len(lengths) == 0:
            msg = ("No periodic read lengths and offsets were found! Are the "
        ...
        filtered_bam = filenames.get_riboseq_bam(config['riboseq_data'],
                                                 sample_name,
                                                 is_unique=is_unique,
                                                 length=lengths,
                                                 note=note)

        cmd = "keep-ribo-periodic {} {} --lengths {} {} ".format(bam_to_filter,
                                                                 filtered_bam,
                                                                 lengths_str,
                                                                 logging_str)
        ...
        file_checkers = {
            filtered_bam: bam_utils.check_bam_file
        }
        shell_utils.call_if_not_exists(cmd, out_files,
                                       in_files=in_files,
