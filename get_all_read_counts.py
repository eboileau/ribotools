...
reads for the RNA pipeline.

*** currently final output is in a different directory, this
is easier since the actual counting script is bash, but we should
incorporate this in the Rp-Bp/B-tea pipeline and use a python script,
so we could get the final files just using regular 'ribo_filenames calls'.

Find final genome files stranded and length adjusted, otherwise will
proceed as usual
...
def main():
    ...
    parser.add_argument('--rna', help="""Optional argument: ribo-seq by default, unless "
        ...
        required='--trimmed-rna' in sys.argv)
