...
# Usage: ./pre-sample-table.R [YAML] [OUTPUT] <CONDITIONS>
# 1: [YAML]       Yaml config file (same as used for run-htseq-workflow)
# 2. [OUTPUT]     Output directory (created if does not exist)
# 3. <CONDITIONS> Comma-separated list of conditions
...
if (length(args)<2) { stop("./pre-sample-table.R [YAML] [OUTPUT] <CONDITIONS>\n", call.=FALSE) }
# config file
...
if (length(args)>2) { conditions <- unique(unlist(strsplit(args[3], ","))) }
...
project <- ""
if (!is.null(params$project_name)) { project <- params$project_name }
write.csv(sampleTable, file=file.path(dirloc.out, paste0("sample-table-", project, ".csv"), fsep=.Platform$file.sep), row.names=FALSE)
