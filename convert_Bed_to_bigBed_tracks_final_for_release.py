...
** The filtered.predicted-orfs.bed.gz files were renamed
previously (01.03.19) and relabelled_orf_type field was added;
however novel_suspect were kept apart. We now include these as
novel_noncoding and thus re-write the field again.
Otherwise remove Other and in-frame p-sites <= 5.
The bigBed files are also renamed.
"""
...
def _get_bed(input_filename, fields_to_keep, filtered_ids, args, labels):
    ...
    # since we did this before, remove before re-writing
    bed_df.drop(columns='relabelled_orf_type', inplace=True)

    bed_df = bed_df.merge(labels[['id', 'relabelled_orf_type']], how='left', on='id')
    # change novel_suspect to novel_noncoding
    bed_df.loc[bed_df['relabelled_orf_type'] == 'novel_suspect', 'relabelled_orf_type'] = 'novel_noncoding'
    ...
    cond_name = filen.split('.')[0]
    new_cond_name = rename_condition[cond_name]
    new_cond_name = new_cond_name + '.orfs'
    #output_filename = os.path.join(args.out, filen)
    output_filename = os.path.join(args.out, new_cond_name)
...
def main():
    ...
    for name in sorted(condition_names.keys()):
        ...
        #bed_file_name, novel_suspect_filename = _get_bed(predicted_orfs, fields_to_keep, filtered_ids, args, labels)
        bed_file_name = _get_bed(predicted_orfs, fields_to_keep, filtered_ids, args, labels)
        bed = bed_file_name + '.tmp.bed'
        bb = bed_file_name + '.bb'
        _convert(bed, bb, use_config_fields, args)
